...
class FormMahasiswaFrame(tk.Frame):
    ...
    def setup_ui(self):
        ...
        self.btn_cari = tk.Button(frame_search, text="Cari Data", bg="#003366", fg="white", font=("Arial", 9, "bold"), width=12, cursor="hand2")
        self.btn_cari.pack(side="left", padx=5)
        self.btn_tampil_semua = tk.Button(frame_search, text="Tampilkan Semua", bg="#555555", fg="white", font=("Arial", 9, "bold"), width=14, cursor="hand2")
        self.btn_tampil_semua.pack(side="left", padx=5)

        # ==================== FORM DATA MAHASISWA ====================
        frame_form = tk.LabelFrame(self, text=" Data Mahasiswa ", font=("Arial", 11, "bold"), padx=15, pady=15, bg="#f0f0f0")
        ...
        self.cmb_jurusan = ttk.Combobox(frame_form, values=["Teknik Informatika", "Sistem Informasi", "Teknik Elektro", "Manajemen", "Akuntansi"], state="readonly", width=23, font=("Arial", 10))
        ...
        self.btn_simpan = tk.Button(frame_btn, text="Daftarkan", bg="#003366", fg="white", width=12, font=("Arial", 10, "bold"), cursor="hand2")
        self.btn_simpan.pack(side="left", padx=8)
        self.btn_update = tk.Button(frame_btn, text="Update", bg="#CC8400", fg="white", width=12, font=("Arial", 10, "bold"), cursor="hand2")
        self.btn_update.pack(side="left", padx=8)
        self.btn_hapus = tk.Button(frame_btn, text="Hapus", bg="#CC0000", fg="white", width=12, font=("Arial", 10, "bold"), cursor="hand2")
        self.btn_hapus.pack(side="left", padx=8)
        self.btn_clear = tk.Button(frame_btn, text="Bersihkan Form", bg="#555555", fg="white", width=14, font=("Arial", 10, "bold"), cursor="hand2")
    ...
    def get_form_data(self):
        return (self.ent_nim.get().strip(), self.ent_nama.get().strip(), self.cmb_jurusan.get().strip(), self.ent_tahun.get().strip())
...
class FormKRSFrame(tk.Frame):
    ...
    def setup_ui(self):
        label_judul = tk.Label(self, text="Form Pengisian KRS", font=("Times New Roman", 18, "bold"), bg="#f0f0f0", fg="#003366")
        label_judul.pack(pady=(20, 10))

        # --- Bagian Pencarian Mahasiswa ---
        frame_cari = tk.Frame(self, bg="#f0f0f0")
        frame_cari.pack(fill="x", padx=20, pady=5)
        
        tk.Label(frame_cari, text="NIM Mahasiswa:", font=("Arial", 10, "bold"), bg="#f0f0f0").pack(side="left", padx=5)
        self.ent_nim_krs = tk.Entry(frame_cari, width=20, font=("Arial", 10))
        self.ent_nim_krs.pack(side="left", padx=5)
        self.btn_cek_mhs = tk.Button(frame_cari, text="Cari KRS", bg="#CC8400", fg="white", cursor="hand2", font=("Arial", 9, "bold"))
        self.btn_cek_mhs.pack(side="left", padx=10)

        # Label Informasi Nama dan SKS
        self.lbl_info_mhs = tk.Label(frame_cari, text="Pilih mahasiswa untuk mulai mengisi KRS.", font=("Arial", 10, "italic"), bg="#f0f0f0", fg="#555")
        self.lbl_info_mhs.pack(side="left", padx=20)
        self.lbl_sks = tk.Label(frame_cari, text="Total SKS: 0", font=("Arial", 10, "bold"), bg="#f0f0f0", fg="#8B0000")
        self.lbl_sks.pack(side="right", padx=10)

        # --- Grid Tabel ---
        frame_tabel_krs = tk.Frame(self, bg="#f0f0f0")
        frame_tabel_krs.pack(fill="both", expand=True, padx=20, pady=10)
        frame_tabel_krs.columnconfigure(0, weight=1)
        frame_tabel_krs.columnconfigure(1, weight=1)
        frame_tabel_krs.rowconfigure(1, weight=1)

        # Label Header Tabel
        tk.Label(frame_tabel_krs, text="Mata Kuliah Tersedia (Klik Ganda untuk Tambah)", font=("Arial", 10, "bold"), bg="#f0f0f0").grid(row=0, column=0, pady=5)
        tk.Label(frame_tabel_krs, text="KRS Anda (Mata Kuliah Terpilih)", font=("Arial", 10, "bold"), bg="#f0f0f0").grid(row=0, column=1, pady=5)
        ...
        self.tree_mk.column("kode", width=80, anchor="center")
        self.tree_mk.column("nama", width=250, anchor="w")
        self.tree_mk.column("sks", width=50, anchor="center")
        self.tree_mk.grid(row=1, column=0, sticky="nsew", padx=(0, 10))
        ...
        self.tree_krs.heading("sks", text="SKS")
        self.tree_krs.column("id", width=30, anchor="center")
        self.tree_krs.column("kode", width=70, anchor="center")
        self.tree_krs.column("nama", width=230, anchor="w")
